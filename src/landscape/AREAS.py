...
class SpeedArea(Area):
    sprite = pygame.image.load('pictures/area/speed_area.png')
...
class SlowArea(Area):
    sprite = pygame.image.load('pictures/area/slow_area.png')
    last_speed = 30
...
class GrowArea(Area):
    sprite = pygame.image.load('pictures/area/grow_area.png')
...
class JewishArea(Area):
    sprite = pygame.image.load('pictures/area/holy_water.png')
    ...
    def out_of_effect(self, snake):
        snake.lifes = snake.lifes - 1 if snake.lifes - 1 >= 0 else snake.lifes
        if snake.lifes == 0:
            snake.alive = False
        snake.coefficient += 1
